...
outputdir=$outputroot/output_kss_tacotron
mkdir -p $outputdir
echo $outputdir
...
OPENBLAS_CORETYPE=nehalem python -m multiproc train.py -m Tacotron2 \
 --learning-rate 1e-3 --epochs 6001 --batch-size 312 --weight-decay 1e-6 --grad-clip-thresh 1.0 \
 --load-mel-from-disk --training-files=$trainfiles --validation-files=$valfiles \
 --log-file nvlog.json --anneal-steps 500 1000 1500 --anneal-factor 0.1 \
 --output $outputdir \

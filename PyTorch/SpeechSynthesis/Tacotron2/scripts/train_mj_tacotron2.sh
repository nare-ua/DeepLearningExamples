...
# aneal to 10% when the epohcs hist 6050 where it seems to start diverge
# when the lr is kept @ 3e-05
mkdir -p output_mj_tactron2_run_fr_em
python -m multiproc train.py -m Tacotron2 -o output_mj_tactron2_run_fr_em/ --checkpoint-path $pretrained --amp-run -lr 1e-6 --epochs 8000 -bs 38 --weight-decay 1e-8 --grad-clip-thresh 1.0 --cudnn-enabled --load-mel-from-disk --training-files=/mnt/data/datasets/MJ/filelists/mj_mel_text_train_filelist.txt --validation-files=/mnt/data/datasets/MJ/filelists/mj_mel_text_val_filelist.txt --log-file nvlog.json --anneal-steps 6050 6300 6500 --anneal-factor 0.1 --use-saved-learning-rate true
